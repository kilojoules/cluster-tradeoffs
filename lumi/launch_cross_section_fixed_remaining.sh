#!/bin/bash
# Run the cases that OOM'd on Sophia CPU (DEI, a=0.5 cases, a=0.7 d2/d5)
# These need GPU for XLA compilation
...
# Also a=0.7 d2 and d5 that failed
for dist in 2 5; do
    sbatch --job-name="xf-a0.7_f-d${dist}" \
           --account=project_465002609 --partition=small-g \
           --gpus-per-node=1 --ntasks=1 --cpus-per-task=7 --mem=60G \
           --time=1-12:00:00 \
...
echo "${count} remaining cross-section jobs submitted on LUMI GPU."
