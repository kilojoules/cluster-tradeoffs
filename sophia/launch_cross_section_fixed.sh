...
do
    ...
    for dist in 2 5 10 15 20 30 40; do
        sbatch --job-name="xf-${case_name:0:6}-d${dist}" \
               ...
               --wrap="cd /work/users/juqu/cluster-tradeoffs && export JAX_PLATFORMS=cpu && export XLA_FLAGS='--xla_cpu_multi_thread_eigen=true intra_op_parallelism_threads=32' && pixi run python scripts/run_regret_cross_section.py --n-bearings 24 --distances-D ${dist} --n-inner-starts 500 --inner-max-iter 5000 --deficit bastankhah --chunk-size 1 ${rose_args} --output-dir analysis/cross_section_fixed/${case_name}_d${dist}"
        count=$((count + 1))
    done
done
