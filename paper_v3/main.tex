...
Regret cross-sections at $K = 100$ confirm an ``ambush effect'': peak vulnerability occurs at oblique angles to the dominant wind direction, not along it.
...
These cross-sections are fully converged ($K = 500$) for four elliptical cases.
The DEI and moderate-bidirectional ($a = 0.5$, $f = 0.0$) cases are currently running and will be added to the final version.
...
\paragraph{Adversarial assumption.}
The greedy grid search provides an upper bound.
Realistic self-interested neighbors may produce different regret patterns; preliminary evidence suggests they can produce \emph{higher} regret at certain bearings because AEP-optimal neighbor layouts concentrate turbines upwind, but this requires further investigation at converged~$K$.
