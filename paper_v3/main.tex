...
Buffer distance analysis shows that regret decay depends on wind rose shape: concentrated unidirectional sites require buffers exceeding $40D$ (9.6\,km), while bidirectional sites need minimal separation.
\end{abstract}
...
Peak bearings are unchanged from K=100 to K=500 for three cases and shift slightly (from 135\,$^\circ$ to 105\,$^\circ$) for the mid-range case (Table~\ref{tab:cross-section}).
...
\begin{table}[t]
    ...
    Dominant wind: 270\,$^\circ$.}
    \label{tab:cross-section}
    \begin{tabular}{lrrrr}
        \toprule
        Case & Max (GWh) & Max (\%) & Bearing & Distance \\
        \midrule
        $a = 0.5$, $f = 1.0$    & 174.1 & 4.42 & 135\,$^\circ$ &  $5D$ \\
        DEI real wind rose       & 143.6 & 2.59 & 225\,$^\circ$ & $15D$ \\
        $a = 0.9$, $f = 1.0$    & 128.6 & 3.24 & 315\,$^\circ$ & $15D$ \\
        $a = 0.7$, $f = 0.5$    & 127.0 & 3.21 & 105\,$^\circ$ & $15D$ \\
        $a = 0.9$, $f = 0.0$    & 105.9 & 2.67 & 195\,$^\circ$ & $60D$ \\
        $a = 0.5$, $f = 0.0$    &  95.3 & 2.42 &  90\,$^\circ$ & $15D$ \\
        \bottomrule
    \end{tabular}
\end{table}

Peak regret consistently occurs at oblique angles to the dominant wind (270\,$^\circ$), not along it.
...
The DEI real wind rose shows peak regret of 143.6\,GWh (2.59\,\%) at bearing 225\,$^\circ$ and $15D$---consistent with the ``ambush effect,'' as the DEI prevailing wind is from the southwest ($\theta_\text{prev} \approx 251°$) and peak regret occurs 26\,$^\circ$ south of the dominant direction.
...
\begin{itemize}
    \item Under bidirectional conditions ($a = 0.5$, $f = 0.0$), the self-interested neighbor produces 60\,\% of the adversarial peak regret (1.45\,\% vs.\ 2.42\,\% of AEP). The adversarial upper bound substantially overstates the realistic risk.
    \item Under mid-range conditions ($a = 0.7$, $f = 0.5$), a similar reduction occurs: 63\,\% of adversarial (2.01\,\% vs.\ 3.21\,\%).
    \item Under concentrated unidirectional conditions ($a = 0.9$, $f = 1.0$), the self-interested neighbor produces \emph{nearly double} the adversarial peak regret: 5.99\,\% vs.\ 3.24\,\% of AEP.
\end{itemize}
...
The self-interested comparison (Section~\ref{sec:results}) shows that the adversarial upper bound overestimates regret for bidirectional sites (60\,\% of adversarial) but \emph{underestimates} it for concentrated unidirectional sites (1.85$\times$ adversarial).
