"""TurboPark variant: regret contours over (a, f) at fixed minimum buffer."""
...
for i, a in enumerate(A_VALS):
    for j, f in enumerate(F_VALS):
        for k, d in enumerate(D_VALS):
            p = Path(f"analysis/buffer_table_tp_funwake/a{a}_f{f}_d{d}/Nt50/results.json")
...
for ax, d in zip(axes, panels):
    ...
    # Per-site offsets to avoid collisions (DEI and Horns Rev nearly coincide)
    site_offsets = {
        "Horns Rev 1": (8, 8),
        "Lillgrund":   (8, 4),
        "Borssele":    (8, 4),
...
fig.suptitle("TurboPark: regret over wind-rose shape space at fixed minimum buffer distance",
             fontsize=12)
out = Path("paper_v3/figures/regret_contour_at_buffer_tp_fw.png")
