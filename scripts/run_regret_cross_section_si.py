...
def main():
    ...
    # Liberal layout with K=100 multistart for robust baseline
    K_lib = min(args.n_inner_starts, 100)
    print(f"\nComputing liberal layout (K_lib={K_lib} starts)...")
    ...
    # =========================================================================
    # STEP 1: Pre-optimize all neighbor layouts sequentially (one per position)
    # This is the non-parallelizable part; each neighbor needs its own SGD.
    # =========================================================================
    n_positions = len(distances_m) * args.n_bearings
    K = args.n_inner_starts

    all_nx = np.zeros((n_positions, n_ref))
    all_ny = np.zeros((n_positions, n_ref))
    pos_info = []

    print(f"\nSTEP 1: Optimizing {n_positions} self-interested neighbor layouts...")
    t0_si = time.time()
    pi = 0
    for di, dist_m in enumerate(distances_m):
        for bi, bearing in enumerate(bearings):
            ...
            all_nx[pi] = np.array(nx)
            all_ny[pi] = np.array(ny)
            pos_info.append((di, bi, float(bearing), float(dist_m)))
            pi += 1
            if pi % 20 == 0:
                elapsed = time.time() - t0_si
                print(f"  Neighbor {pi}/{n_positions}: {elapsed:.0f}s elapsed")

    all_nx_jnp = jnp.array(all_nx)
    all_ny_jnp = jnp.array(all_ny)
    print(f"  Neighbor optimization complete: {time.time() - t0_si:.0f}s")

    # =========================================================================
    # STEP 2: Vmap-batched conservative optimization over (position × start)
    # =========================================================================
    print(f"\nSTEP 2: Building {K} start layouts and batching...")
    ...
    batch_nx = jnp.repeat(all_nx_jnp, K, axis=0)
    batch_ny = jnp.repeat(all_ny_jnp, K, axis=0)
    ...
    for start in range(0, n_total, CHUNK):
        ...
        rate = elapsed / end if end > 0 else 0
        remaining = rate * (n_total - end)
        pct_done = 100 * end / n_total
        print(f"  Chunk {start:>6}-{end:>6}  ({pct_done:>5.1f}%)  "
              f"elapsed={elapsed/60:.1f}min  eta={remaining/60:.1f}min")

    cons_aeps = all_cons_aeps.reshape(n_positions, K)

    # Compute regret per position with pooling
    ...
    for pi, (di, bi, bearing, dist_m) in enumerate(pos_info):
        ...
        best_cons = max(best_cons, lib_aep_present)  # pool
    ...
    print(f"\nTotal time: {time.time() - t0_si:.0f}s")
