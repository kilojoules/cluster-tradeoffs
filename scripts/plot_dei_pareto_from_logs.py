"""Generate Pareto plot for DEI Farm 8 from log files."""
...
LOG_FILE = Path(__file__).parent.parent / "analysis/dei_50starts_2000iter/farm8.log"
OUTPUT_FILE = Path(__file__).parent.parent / "docs/figures/dei_pareto_farm8.png"
...
def parse_log(log_file):
    ...
    with open(log_file, 'r') as f:
        for line in f:
            # Match lines like: "  Start 0: Liberal=8311.2/8273.4, Conservative=8308.4/8295.5 GWh"
            match = re.search(
...
def compute_pareto(points):
    ...
    for i in range(n):
        for j in range(n):
            if i != j:
                # j dominates i if j >= i in both and > in at least one
...
def main():
    ...
    ax.scatter(all_absent[lib_mask & ~pareto_mask], all_present[lib_mask & ~pareto_mask],
               s=100, c='#3498db', alpha=0.4, label='Liberal (dominated)', marker='o')
    ax.scatter(all_absent[con_mask & ~pareto_mask], all_present[con_mask & ~pareto_mask],
               s=100, c='#e74c3c', alpha=0.4, label='Conservative (dominated)', marker='s')
    ...
    mid_x = all_absent[lib_opt_idx] - 3
    ax.annotate('', xy=(mid_x, all_present[con_opt_idx]),
                xytext=(mid_x, all_present[lib_opt_idx]),
                arrowprops=dict(arrowstyle='<->', color='#e74c3c', lw=2.5))
    ax.text(mid_x - 2, (all_present[lib_opt_idx] + all_present[con_opt_idx])/2,
    ...
    ax.set_title(f'Pareto Front: Farm 8 (South, 163°)\n{pareto_mask.sum()} Pareto points from 100 optimizations (50 starts × 2 strategies)',
