"""Quick diagnostic: compare IFT gradient vs outer FD for the 50x50 DEI problem."""
import jax
...
print(f"Setup: {N_TARGET} targets, {N_NEIGHBOR} neighbors, {N_SAMPLES} wind samples")
...
def compute_aep(target_x, target_y, nb_x=None, nb_y=None):
    ...
    power_out = result.power()[:, :N_TARGET]
    return jnp.sum(power_out) * HOURS_PER_SAMPLE / N_YEARS / 1e6
...
# Liberal baseline
print("\nComputing liberal layout...", flush=True)
t0 = time.time()
...
# IFT gradient
print("\nComputing IFT gradient...", flush=True)
t0 = time.time()
regret_val, ift_grad = jax.value_and_grad(compute_regret)(neighbor_params)
ift_time = time.time() - t0
print(f"Regret = {float(regret_val):.4f} GWh, |grad| = {float(jnp.linalg.norm(ift_grad)):.6f}")
print(f"IFT time: {ift_time:.1f}s", flush=True)

# Outer FD check on first 5 parameters
print("\nOuter FD gradient check (first 5 params)...", flush=True)
eps = 10.0  # 10m perturbation
n_check = 5
for i in range(n_check):
    p_plus = neighbor_params.at[i].set(neighbor_params[i] + eps)
    p_minus = neighbor_params.at[i].set(neighbor_params[i] - eps)

    t0 = time.time()
    r_plus = compute_regret(p_plus)
    r_minus = compute_regret(p_minus)
    fd_time = time.time() - t0

    fd_grad_i = float(r_plus - r_minus) / (2 * eps)
    ift_grad_i = float(ift_grad[i])

    if abs(fd_grad_i) > 1e-12:
        ratio = ift_grad_i / fd_grad_i
    else:
        ratio = float('inf')

    param_type = "nb_x" if i < N_NEIGHBOR else "nb_y"
    idx = i if i < N_NEIGHBOR else i - N_NEIGHBOR
    print(f"  param[{i}] ({param_type}[{idx}]): IFT={ift_grad_i:+.6e}  FD={fd_grad_i:+.6e}  ratio={ratio:.2f}  ({fd_time:.1f}s)")
