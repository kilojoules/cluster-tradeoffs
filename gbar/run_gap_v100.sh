...
#BSUB -W 24:00
#BSUB -u /dev/null
#BSUB -o /zhome/5c/f/180689/clusters_logs/gap.%J.%I.out
#BSUB -e /zhome/5c/f/180689/clusters_logs/gap.%J.%I.err
...
CSV=$HOME/clusters_jobs/gbar_gap_jobs.csv
LINE=$((LSB_JOBINDEX + 1))
ROW=$(sed -n "${LINE}p" "$CSV" | tr -d '\r')
if [ -z "$ROW" ]; then
    echo "[gap] empty row at idx=$LSB_JOBINDEX" >&2
    exit 1
fi
...
echo "[gap] idx=$LSB_JOBINDEX study=$STUDY wake=$WAKE key=$NAME"
echo "[gap] outdir=$OUTDIR"
echo "[gap] args=$PYARGS"
echo "[gap] node=$(hostname) work=$WORK date=$(date)"
...
echo "[gap] DONE rc=$RC result -> $DEST date=$(date)"
exit $RC
