...
#BSUB -W 72:00
#BSUB -u /dev/null
#BSUB -o /zhome/5c/f/180689/clusters_logs/fwfill.%J.%I.out
#BSUB -e /zhome/5c/f/180689/clusters_logs/fwfill.%J.%I.err

# Per-cell array job for FunWake gbar backfill.
# Each task reads its (study, wake, name, outdir, python_args) from
# ~/clusters_jobs/gbar_funwake_jobs.csv at LSB_JOBINDEX,
# clones the repo fresh into /tmp/$LSB_JOBID, runs analysis,
# and rsyncs results.json to ~/clusters_results/.
...
CSV=$HOME/clusters_jobs/gbar_funwake_jobs.csv
LINE=$((LSB_JOBINDEX + 1))   # +1 for header
ROW=$(sed -n "${LINE}p" "$CSV" | tr -d '\r')
if [ -z "$ROW" ]; then
    echo "[fwfill] empty row at idx=$LSB_JOBINDEX" >&2
    exit 1
fi
...
WALLHR=$(echo "$ROW" | cut -d',' -f6)
PYARGS=$(echo "$ROW" | cut -d',' -f7-)

echo "[fwfill] idx=$LSB_JOBINDEX  study=$STUDY  wake=$WAKE  name=$NAME"
echo "[fwfill] outdir=$OUTDIR"
echo "[fwfill] args=$PYARGS"
echo "[fwfill] node=$(hostname)  work=$WORK  date=$(date)"
...
echo "[fwfill] DONE  rc=$RC  result -> $DEST  date=$(date)"
exit $RC
