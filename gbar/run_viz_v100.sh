...
#BSUB -W 24:00
#BSUB -u /dev/null
#BSUB -o /zhome/5c/f/180689/clusters_logs/viz.%J.%I.out
#BSUB -e /zhome/5c/f/180689/clusters_logs/viz.%J.%I.err
...
CSV=$HOME/clusters_jobs/gbar_viz_jobs.csv
LINE=$((LSB_JOBINDEX + 1))
ROW=$(sed -n "${LINE}p" "$CSV" | tr -d '\r')
if [ -z "$ROW" ]; then
    echo "[viz] empty row at idx=$LSB_JOBINDEX" >&2
    exit 1
fi
...
echo "[viz] idx=$LSB_JOBINDEX study=$STUDY wake=$WAKE key=$NAME"
echo "[viz] outdir=$OUTDIR"
echo "[viz] args=$PYARGS"
echo "[viz] node=$(hostname) work=$WORK date=$(date)"
...
echo "[viz] DONE rc=$RC result -> $DEST date=$(date)"
exit $RC
