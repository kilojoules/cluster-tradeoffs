...
#BSUB -W 24:00
#BSUB -u /dev/null
#BSUB -o /zhome/5c/f/180689/clusters_logs/grev.%J.%I.out
#BSUB -e /zhome/5c/f/180689/clusters_logs/grev.%J.%I.err
...
CSV=$HOME/clusters_jobs/gbar_greedy_reeval.csv
LINE=$((LSB_JOBINDEX + 1))
ROW=$(sed -n "${LINE}p" "$CSV" | tr -d '\r')
if [ -z "$ROW" ]; then
    echo "[grev] empty row at idx=$LSB_JOBINDEX" >&2
    exit 1
fi
...
echo "[grev] idx=$LSB_JOBINDEX study=$STUDY wake=$WAKE key=$NAME"
echo "[grev] outdir=$OUTDIR"
echo "[grev] args=$PYARGS"
echo "[grev] node=$(hostname) work=$WORK date=$(date)"
...
# stage the greedy source results the re-evaluation reads (analysis/ is not in git)
mkdir -p analysis
cp -r $HOME/clusters_jobs/greedy_sources/* analysis/
echo "[grev] staged sources: $(find analysis -name results.json | wc -l) files"
...
echo "[grev] DONE rc=$RC result -> $DEST date=$(date)"
exit $RC
