...
# Sanity test: K=50 reduced run on a single buffer_table cell.
# Verifies clone + pixi cuda install + JAX GPU + run script end-to-end
# in ~30-60 min before launching full array.
...
mkdir -p $WORK/jax_cache $WORK/jax_autotune
export JAX_COMPILATION_CACHE_DIR=$WORK/jax_cache
export XLA_FLAGS="--xla_gpu_per_fusion_autotune_cache_dir=$WORK/jax_autotune"
