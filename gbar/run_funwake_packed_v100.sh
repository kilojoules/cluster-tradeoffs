...
#BSUB -W 24:00
#BSUB -u /dev/null
#BSUB -o /zhome/5c/f/180689/clusters_logs/fwpackv.%J.%I.out
#BSUB -e /zhome/5c/f/180689/clusters_logs/fwpackv.%J.%I.err
...
CSV=$HOME/clusters_jobs/gbar_packed_jobs_short_v100.csv
LINE=$((LSB_JOBINDEX + 1))
ROW=$(sed -n "${LINE}p" "$CSV" | tr -d '\r')
if [ -z "$ROW" ]; then
    echo "[fwpackv] empty row at idx=$LSB_JOBINDEX" >&2
    exit 1
fi
...
echo "[fwpackv] idx=$LSB_JOBINDEX study=$STUDY wake=$WAKE key=$NAME"
echo "[fwpackv] outdir=$OUTDIR"
echo "[fwpackv] args=$PYARGS"
echo "[fwpackv] node=$(hostname) work=$WORK date=$(date)"
...
echo "[fwpackv] DONE rc=$RC result -> $DEST date=$(date)"
exit $RC
