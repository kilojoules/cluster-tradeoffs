...
#BSUB -W 24:00
#BSUB -u /dev/null
#BSUB -o /zhome/5c/f/180689/clusters_logs/cs500dei.%J.%I.out
#BSUB -e /zhome/5c/f/180689/clusters_logs/cs500dei.%J.%I.err
...
CSV=$HOME/clusters_jobs/gbar_cs_k500_dei_fix.csv
LINE=$((LSB_JOBINDEX + 1))
ROW=$(sed -n "${LINE}p" "$CSV" | tr -d '\r')
if [ -z "$ROW" ]; then
    echo "[cs500dei] empty row at idx=$LSB_JOBINDEX" >&2
    exit 1
fi
...
echo "[cs500dei] idx=$LSB_JOBINDEX study=$STUDY wake=$WAKE key=$NAME"
echo "[cs500dei] outdir=$OUTDIR"
echo "[cs500dei] args=$PYARGS"
echo "[cs500dei] node=$(hostname) work=$WORK date=$(date)"
...
echo "[cs500dei] DONE rc=$RC result -> $DEST date=$(date)"
exit $RC
