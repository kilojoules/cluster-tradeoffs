...
#BSUB -W 72:00
#BSUB -u /dev/null
#BSUB -o /zhome/5c/f/180689/clusters_logs/fwpacka.%J.%I.out
#BSUB -e /zhome/5c/f/180689/clusters_logs/fwpacka.%J.%I.err
...
CSV=$HOME/clusters_jobs/gbar_packed_jobs_long_a100.csv
LINE=$((LSB_JOBINDEX + 1))
ROW=$(sed -n "${LINE}p" "$CSV" | tr -d '\r')
if [ -z "$ROW" ]; then
    echo "[fwpacka] empty row at idx=$LSB_JOBINDEX" >&2
    exit 1
fi
...
echo "[fwpacka] idx=$LSB_JOBINDEX study=$STUDY wake=$WAKE key=$NAME"
echo "[fwpacka] outdir=$OUTDIR"
echo "[fwpacka] args=$PYARGS"
echo "[fwpacka] node=$(hostname) work=$WORK date=$(date)"
...
echo "[fwpacka] DONE rc=$RC result -> $DEST date=$(date)"
exit $RC
